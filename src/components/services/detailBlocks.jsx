...
export function ServiceCTA({ title, text, button = "Оставить заявку", to = "/contact", backTo = "/services/electrical", backLabel = "Вернуться к электромонтажу" }) {
  // На мобилке кнопка морковная; на ПК — прозрачная (ghost), заливается тёмным
  // при наведении, чтобы единственный морковный акцент оставался в стики-доке.
    ...
    <CubeCard>
      ...
      <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" }}>
        <button
          ...
          onMouseEnter={(e) => {
            if (isDesktop) { e.currentTarget.style.background = "#111"; e.currentTarget.style.color = "#fff"; }
            else { e.currentTarget.style.background = "#e8541f"; }
          }}
          onMouseLeave={(e) => {
            if (isDesktop) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = INK; }
            else { e.currentTarget.style.background = "#FA5D29"; }
          }}
          style={{
            display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8,
            height: 46, padding: "0 20px", borderRadius: 10,
            border: isDesktop ? `1px solid ${INK}` : "none",
            background: isDesktop ? "transparent" : "#FA5D29",
            color: isDesktop ? INK : "#000",
            fontFamily: UI, fontSize: 15, fontWeight: 600,
            cursor: "pointer", whiteSpace: "nowrap", transition: "background-color .16s ease, color .16s ease",
          }}
        >
          {button}
        </button>
