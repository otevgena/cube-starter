...
/* ====== BACKEND URL (как в секции на главной) ====== */
...
/* ====== Глобальный сдвиг страницы вниз ====== */
const GLOBAL_SHIFT = 42;

/* ====== Валидации ====== */
const isEmail = (v) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test((v || "").trim());
const isPhoneLike = (v) => ((v || "").replace(/\D/g, "").length >= 10);

/* ====== Слот ошибки под полем ====== */
...
export default function ContactPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      console.log("[contact-page] POST →", url, payload);
      ...
      console.log("[contact-page] resp", res.status, text);
  ...
  return (
    <main style={{ fontFamily: UI, color: BLACK, background: "#f8f8f8" }} aria-label="Контакты">
      {/* Верхняя зона приподнята — СДВИГНУЛИ ВНИЗ НА 42px (−98 → −56) */}
      <div style={{ transform: `translateY(-56px)`, willChange: "transform" }}>
        {/* Шапка */}
        <div className="about-hero-flow" style={{ marginTop: 30, textAlign: "center" }}>
          <div style={{ fontSize: 14, lineHeight: "28px", fontWeight: 300, color: GRAY, margin: 0 }}>
            Напишите нам
          </div>
      ...
      {/* ===== Всплывающий ЧЁРНЫЙ БЛОК по центру — тоже опущен на 42px ===== */}
      {modal && (
        <div
          role="status"
          aria-live="assertive"
          style={{
            ...
            pointerEvents: "none",
            transform: `translateY(${GLOBAL_SHIFT}px)`,
          }}
        >
          <div
            style={{
