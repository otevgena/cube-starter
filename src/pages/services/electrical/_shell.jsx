// src/pages/services/electrical/_shell.jsx
// Верх детальных страниц «Электромонтаж»: вкладки + заголовок + слоган +
// «Обновление» и низ «← Все услуги». Тело страниц — из detailBlocks.jsx.
...
// боковые поля тела: на десктопе те же 80px, на узких экранах ужимаются (без горизонтального скролла)
const PAD = "clamp(16px, 6vw, 80px)";
export { UI, BG, INK };

const TABS = [
  { label: "Подключение объектов к электросетям", href: "/services/electrical/power-connection" },
  { label: "Увеличение мощности и модернизация сетей", href: "/services/electrical/power-upgrade" },
  { label: "Внутренние электромонтажные работы", href: "/services/electrical/indoor" },
  { label: "Наружные электросети и уличное освещение", href: "/services/electrical/outdoor-networks" },
  { label: "Монтаж электрощитов и ВРУ", href: "/services/electrical/switchgear-vru" },
  { label: "Системы заземления и молниезащиты", href: "/services/electrical/earthing-lightning" },
  { label: "Автоматизация и учёт электроэнергии", href: "/services/electrical/energy-metering-automation" },
  { label: "Резервное электроснабжение", href: "/services/electrical/backup-power" },
];
    ...
    <main style={{ background: BG, minHeight: "100dvh", fontFamily: UI, color: INK }}>
      ...
      <div className="-mt-12 text-center will-change-transform lg:mt-0 lg:-translate-y-[61px]">
        ...
        <div className="px-4 pt-2 text-left lg:hidden">
          <SpaLink to="/services/electrical" style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 14, fontWeight: 300, textTransform: "uppercase", color: "#a7a7a7", textDecoration: "none" }}>
        ...
        <h1
          className="mt-2 px-4 text-[clamp(24px,8.5vw,34px)] sm:text-[clamp(48px,13.5vw,137px)] md:text-[clamp(40px,6vw,52px)] lg:mt-0.5 lg:px-0 lg:text-[clamp(44px,7.5vw,88px)] xl:text-[clamp(48px,13.5vw,137px)]"
      ...
      <div data-reveal-seq className="mx-[clamp(16px,6vw,80px)] mt-10 grid gap-y-12 md:mx-4 lg:mx-[80px] lg:mt-20 lg:gap-y-[72px]">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4" style={{ marginTop: 24 }}>
          <div style={{ color: "#3b3b3b", fontSize: 14, fontWeight: 300 }}>Страница услуги в составе раздела «Электромонтаж».</div>
          <Capsule to="/services/electrical">← Все услуги электромонтажа</Capsule>
        </div>
      </div>
